...
> **THE SCOPE RULING WAS OVERTURNED — 2026-08-09, Dustin's call: *"make sure that all of them have a little explanation, because most people don't know what most of this stuff is."*** `why` goes **23 → 50 of 51**, and the paragraph above is the record of the ruling it replaces, not what ships.
>
> **The 8 → 28 estimate it was kept on is stale, and the real price is a third of it.** That figure was measured before ruling H made every subject draw at one of two page constants and before ruling I cut `padOf`. Re-measured on the shipped page with all 27 notes written: **`art dropped` 11 → 10 at each desktop 200 % variant** — the desktop *improves*, because a card that drops its line to fit is shorter than one that keeps it — **4 → 12 at 390×844/200 % and 4 → 10 at 390×780/200 %**, and **0 → 0 at all four 100 % variants**. The page-wide size constants go **190.7/162.1 → 178.6/151.8** desktop (−6.3 %), **unchanged at 390×844**, and 167.9/142.8 → 161.9/137.6 at 390×780. Phones at 200 % text pay the whole bill, which is [§10](#10--accessibility)'s own ruling direction — *text costs art* — on the viewport the note exists for.
>
> **One card is exempt, and it is not an oversight: `sex`.** A note there takes it to **77.9 % of its tier, under `ART_SIZE_SHORTFALL_MIN` (0.78), and the collision gate fails all four 100 % variants.** Its aspect (0.4) already binds the whole page's picture size ([§5](#5--the-no-collision-layout-contract) rule 2), so it is the one card with no row to spare — and it is also the one card whose name is not a science word. Loosening the gate to fit copy would be backwards. Revisit with the open square-trim ruling, not before.
>
> **The budget did not move: 52 plain characters.** The 27 new notes run 39–53.
...
> **THE SLOT IS REFILLED — later the same day, Dustin's call: *"the ending needs to bring home the size a little bit more to make people understand how little humans have been around on Earth. Maybe use some comparisons."*** What was cut above is the epilogue's **subject** — the aside about the page's own editing — and that stays cut. What ships in the slot is the humans, restated in a unit the visitor already owns:
>
...
>
> **It had to be this slot, and that is arithmetic, not preference.** The comparison cannot ride on `copy.closing`: at 1440×900/200 % text the closing's block already takes **772.1 of a 900 px viewport**, and the shortest usable extra clause (+49 characters) takes the band to **889.9** and leaves the flood field 5 px. The epilogue is solved as a `max` against the closing and set two steps smaller (13 px against `clamp(14px, 1.5vw, 19px)`), so **200 characters there cost the band exactly zero at all eight gate variants**. 200 is the budget — 206 takes a row on both phones — and it is pinned in `timeline.test.ts`.
>
> **The cost, accepted and flagged rather than smuggled:** the slot has two tenants again, so the swap below is live again and the page ends on the epilogue rather than on the closing line. That is the one thing the ruling above decided the other way. **Reverting is `copy.epilogue` back to `""` — nothing else moves**, which is exactly what the empty-string discipline below was kept for.
>
> Every number in it is recomputed from `withheld` and `constants` by the tests: 4.6 Ga as 365 days puts *Homo sapiens* 34.3 minutes before midnight, farming 82.3 seconds, writing 37.0, the industrial revolution 1.7.
...
| **left holding** | 6,725 → 7,000 | 0.6 s | **The epilogue** — the calendar-year comparison — and `↑ again`. Last state; holds indefinitely. |
...
**The thirteen ship as baked — ruled 2026-08-09**, on Dustin's call after reviewing all 13 regenerated subjects *on the built page at HEAD*, at 1440×900 and 390×844, mid-dwell. **Every one is kept.** Two objections are recorded and **accepted rather than acted on**: `first-ice-age` renders its haze as a rim ring rather than a veil over the disc and carries a near-black outline around it — this section's own named failure for that subject, *surviving its own negative* — and `plants-reach-land` is saturated and gloss-shaded where the prompt asked for pale washed translucent amber lit from behind. Smaller notes, also accepted: a painted dark rim on `first-continents` and `great-oxidation-ends`, a level-edged melt band on `ice-retreats`, and `moon-torn-out` still close to the molten-Earth portrait 750 px earlier. **Accepted is not settled** — anything here that is ever regenerated must clear both the 3:1 gate and the collision gate on the way back in, and the two named failures are the proof this recipe's negatives do not always hold on their own.

**A subject must be judged on the page, not on the asset, and not against a stale sheet.** The review sheet built on 2026-08-08 showed these subjects ~37 % smaller than they ship, because [ruling I](#ruling-i--the-cap-was-in-the-bake-not-the-layout) landed between the sheet and the reading. Capture from the built page at the commit being judged.
...
8. **A planet is trimmed to a SQUARE about its own disc — ruled 2026-08-09**, after baking the page both ways at HEAD and gating each. *Added 2026-08-08; the ruling is the keep.* **Without the trim the page fails `gate:collision` at all four 100 %-text viewports** on rule 1: the lead falls to **1.18 / 1.18 / 1.15 / 1.09** against ruling I's 1.25 floor, and **nine of the ten portraits shrink** — worst `great-oxidation-ends` at **−26 %** (303.4 → 224.4 px), then `ice-retreats` and `ice-breaks-for-good` at −21 %, and only `chicxulub` unmoved. Transfer would fall 3.435 → 3.333 MB and the tightest 3:1 contrast is 3.09 either way, so the trade is 0.10 MB against the gate. **A third bake settles what the *square* itself buys**, as distinct from the trim: trimming planets *rectangularly* instead lands within ~1 % of the square on every portrait (largest gap `ice-breaks-for-good`, 302.6 → 306.4). The size is bought by trimming **at all**; the square buys this section's own claim — *a circle has no aspect ratio, so the same asset composes identically at 1440×900 and 390×844* — as a **guarantee rather than a coincidence**, at no measurable cost, and a stray keyed speck can no longer quietly turn a portrait into a rectangle.

**Rule 3 holds on a phone too, and no mobile carve-out bought it. Added 2026-08-09.**

Measured 2026-08-08, before [ruling I](#ruling-i--the-cap-was-in-the-bake-not-the-layout): **six of the ten portraits at 390×844 and seven at 390×780 drew under the biggest ordinary card**, against zero at both desktop widths. The amendment drafted that day would have scoped rule 3's size claim to desktop and named the phone an exception.

**It is not written, because ruling I removed the thing it described.** Re-measured at HEAD: **0 of 10 under the biggest card at all four gate viewports** — smallest portrait 260.4 against a 190.7 card on desktop, 248.0 against 181.5 at 390×844, 217.0 against 167.9 at 390×780, for a lead of **1.37 / 1.37 / 1.37 / 1.29**. Nothing about mobile was special-cased; `padOf` 0.12 → 0.05 lifted every portrait at every width at once. **A defect measured before a ruling that moved the same numbers is not evidence about the page that ruling left** — the same trap the square-trim decision fell into, where 08-08's *three portraits move* is nine at HEAD.

**The phone is the binding case, and the mechanism is the column's width.** A phone box is `321×449` against desktop's `1290×498`, and **five of the ten portraits draw at exactly 321 px — the box's full width**. Desktop clamps nothing on width: there the disc is held by what the text block leaves it (294–365 px of art inside 498 px, under a text block of 63–134 px). So the phone's ceiling is **fixed by the layout** while the biggest card can still grow toward it, which is why 390×780 carries the tightest lead at 1.29 and is the column to watch on any future copy or art change.

**Gated at every viewport, not only on desktop.** [§5](#5--the-no-collision-layout-contract)'s three art rules — rule 1's 1.25 lead among them — are asserted at 100 % text on all four gate viewports, phones included. Verified by perturbation 2026-08-09: baking the portraits without their square trim drops the lead to **1.148 at 390×844 and 1.086 at 390×780** and fails the gate at both, alongside the two desktop columns.
